...
export default function ClientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <DashboardLayout
      ...
      subtitle={client ? `Client ID: #${id}` : undefined}
      action={backButton}
    >
      {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-card border border-border/80 rounded-2xl p-5 shadow-xs flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-muted-foreground font-medium mt-0.5">Lifetime orders placed</p>
              </div>
            </div>
            ...
            <div className="bg-card border border-border/80 rounded-2xl p-5 shadow-xs flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-muted-foreground font-medium mt-0.5">Aggregated payments</p>
              </div>
            </div>
            ...
            <div className="bg-card border border-border/80 rounded-2xl p-5 shadow-xs flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-muted-foreground font-medium mt-0.5">Fulfilled successfully</p>
              </div>
            </div>
            ...
            <div className="bg-card border border-border/80 rounded-2xl p-5 shadow-xs flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[11px] text-muted-foreground font-medium mt-0.5">In production / pending</p>
              </div>
            </div>
          </div>
              ...
              <div className="bg-card border border-border/80 rounded-2xl shadow-xs overflow-hidden">
                ...
                <div className="flex items-center border-b border-border/80 bg-muted/30 px-6 pt-3 gap-6 overflow-x-auto">
                  <button
                    ...
                    className={`pb-3 font-bold text-xs border-b-2 transition-all cursor-pointer whitespace-nowrap flex items-center gap-2 ${
                      activeTab === "orders"
                              ...
                              return (
                                <tr key={ord.id} className="hover:bg-muted/20 transition-colors">
                                  ...
                                  <td className="py-3.5 px-5 text-muted-foreground font-mono text-[11px]">
                                    {ord.created_at ? new Date(ord.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : 'N/A'}
                                  </td>
                        ...
                        <table className="w-full text-xs text-left">
                          ...
                          <tbody className="divide-y divide-border/40">
                            {transactions.map((txn, idx) => {
                              const txnId = txn.transaction_number || txn.razorpay_payment_id || txn.payment_id || txn.transaction_id || `TXN-${txn.id || idx}`;
                              ...
                              return (
                                <tr key={txn.id || idx} className="hover:bg-muted/20 transition-colors">
                                  ...
                                  <td className="py-3.5 px-5 font-mono text-muted-foreground">
                                    {txn.order_number || (txn.pcb_order_id ? `#ORD-${txn.pcb_order_id}` : 'N/A')}
                                  </td>
                                  ...
                                  <td className="py-3.5 px-5 text-muted-foreground font-mono text-[11px]">
                                    {txn.created_at || txn.date ? new Date(txn.created_at || txn.date || '').toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : 'N/A'}
                                  </td>
                                </tr>
                              );
                            })}
