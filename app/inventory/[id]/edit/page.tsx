...
export default function EditInventoryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <DashboardLayout title="Edit Component Details" subtitle="Update component specs, price & stock threshold" action={backButton}>
      <div className="w-full space-y-6 animate-in fade-in duration-300">
        {/* Header Card */}
        <div className="bg-card border border-border/80 rounded-2xl p-6 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
          <div className="absolute top-0 right-0 w-96 h-96 bg-blue-500/5 rounded-full blur-3xl -z-10 pointer-events-none" />

          <div className="flex items-center gap-5">
            <div className="w-14 h-14 rounded-2xl bg-blue-500/10 border border-blue-500/20 text-blue-500 font-black text-xl flex items-center justify-center shrink-0 shadow-inner">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground font-mono font-bold">#{item.id}</span>
              </div>
              <h1 className="text-xl font-black text-foreground mt-1">{item.name}</h1>
              <p className="text-xs text-muted-foreground font-medium mt-0.5">Modify component parameters in central catalog</p>
            </div>
          </div>
        </div>
