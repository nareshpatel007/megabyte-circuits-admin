...
import { Search, X, Check, Loader2, History, ChevronDown } from "lucide-react";

export interface OldOrderItem {
    id: number;
    order_number: string;
...
interface OldOrderSelectProps {
    currentOrderId?: number | null;
    currentOrderNumber?: string | null;
    value: OldOrderItem[];
    onChange: (items: OldOrderItem[]) => void;
    placeholder?: string;
}

export function OldOrderSelect({
    currentOrderId,
    currentOrderNumber,
    value = [],
    onChange,
    ...
    const [options, setOptions] = useState<OldOrderItem[]>([]);
        ...
        const timer = setTimeout(async () => {
            ...
            try {
                ...
                if (active && (json.status || json.success) && Array.isArray(json.data)) {
                    const list: OldOrderItem[] = json.data.map((o: any) => ({
                        id: o.id,
                ...
                console.error("Failed to fetch orders for old order selector:", err);
            } finally {
    ...
    const handleToggle = (item: OldOrderItem) => {
        if (
    ...
    const handleRemove = (e: React.MouseEvent, item: OldOrderItem) => {
        e.stopPropagation();
        onChange(value.filter((v) => v.id !== item.id && v.order_number.toUpperCase() !== item.order_number.toUpperCase()));
    };

    const isSelected = (item: OldOrderItem) => {
        return value.some((v) => v.id === item.id || v.order_number.toUpperCase() === item.order_number.toUpperCase());
    };

    const isCurrent = (item: OldOrderItem) => {
        return (
            (currentOrderId && item.id === currentOrderId) ||
            (currentOrderNumber && item.order_number.toUpperCase() === currentOrderNumber.toUpperCase())
        );
        ...
        <div ref={containerRef} className="relative w-full">
            ...
            <button
                ...
                className={`w-full min-h-[40px] px-3.5 py-2 text-xs bg-white border border-slate-300 rounded-xl text-slate-900 shadow-xs flex items-center justify-between hover:border-slate-400 transition-all focus:outline-none ${
                    isOpen ? "ring-2 ring-amber-500/20 border-amber-500 shadow-sm" : ""
                }`}
            >
                    ...
                    {value.length > 0 ? (
                        value.map((item) => (
                            <span
                                ...
                                className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-bold rounded-lg bg-amber-50 text-amber-800 border border-amber-200/90 shadow-2xs"
                            >
                                ...
                                <span
                                    ...
                                    className="hover:text-amber-950 transition-colors p-0.5 rounded-md hover:bg-amber-200/60 ml-0.5 cursor-pointer"
                                >
                                    <X className="w-3 h-3" />
                                </span>
                            </span>
                ...
                <ChevronDown className={`w-4 h-4 text-slate-400 shrink-0 transition-transform duration-200 ml-1 ${isOpen ? "rotate-180 text-amber-600" : ""}`} />
            </button>
                                    ...
                                    >
                                        <div className="min-w-0 pr-2">
                                            <div className="flex items-center gap-2">
                                                ...
                                                {current && (
                                                    <span className="text-[10px] px-2 py-0.5 rounded-md bg-amber-100 text-amber-800 border border-amber-300 font-normal">
                                                        Current Order
                                                    </span>
                                                )}
                                                {item.status && !current && (
                                        ...
                                        {selected && (
                                            <div className="p-1 rounded-full bg-amber-100 text-amber-700 shrink-0 ml-2">
